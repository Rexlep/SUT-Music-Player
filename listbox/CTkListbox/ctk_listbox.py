...
class CTkListbox(customtkinter.CTkScrollableFrame):
    ...
    def __init__(self,
                 ...
                 select_color: str = "#909090",
                 hover_color: str = "#696969",
                 border_width: int = 3,
                 font: tuple = "default",
    ...
    def get_selected_song(self):
        ...
        Returns:
            str: The text of the selected song
        """
    ...
    def delete(self, index, last=None):
        ...
        if str(index).lower() == "end":
            ...
            if int(index) == len(self.buttons):
                # If the index is equal to the last index, convert it to the actual index
                index = len(self.buttons) - 1
            if int(index) >= len(self.buttons):
                # If the index is out of range, return
                return
            if not last:
                # If 'last' is not provided, convert the index to the actual index
                index = list(self.buttons.keys())[int(index)]

        if last:
            if str(last).lower() == "end":
                # Delete options from the specified index to the last option
                last = len(self.buttons) - 1
            elif int(last) >= len(self.buttons):
                # If 'last' is out of range, set it to the last index
                last = len(self.buttons) - 1

            deleted_list = []
            for i in range(index, int(last) + 1):
                # Destroy the options and collect the keys to delete from the dictionary
                list(self.buttons.values())[i].destroy()
                deleted_list.append(list(self.buttons.keys())[i])
            for i in deleted_list:
                # Delete the options from the dictionary
                del self.buttons[i]
        else:
    ...
    def selection_set(self, current_song_index):
        pass

    def see(self, current_song_index):
        pass

    def select_item(self, item):
        """
        Select a specific item in the listbox by its text value.
        Args:
            item (str): The text value of the item to select.
        """
